...
export function JavaRuntime({ children }: { children: ReactNode }) {
  ...
  const cheerpjUrl = VMStore.useState((s) => s.cheerpjUrl)

  useEffect(() => {
    if (VMStore.getRawState().controllerState == 'loading' && !cheerpjUrl) {
      void (async () => {
        const url = (
          await (
  ...
  async function compileAndRun() {
    ...
    if (ui.dirtyClasses.length > 0) {
      ...
      cheerpOSAddStringFile(`/str/Data/Hilfen.txt`, encoder.encode(Hilfen))
      cheerpOSAddStringFile(
